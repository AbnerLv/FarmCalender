package com.pyp.farmcalender.dao;

import java.util.ArrayList;

import com.pyp.farmcalender.entity.ScheduleDateTag;
...
public class ScheduleDAO {
	 ...
	 * @param scheduleVO
	 */
	public int save(ScheduleEntity scheduleVO){
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
	...
	public ScheduleEntity getScheduleByID(int scheduleID){
		...
		if(cursor.moveToFirst()){
			...
			cursor.close();
			return new ScheduleEntity(schID,scheduleTypeID,remindID,scheduleContent,scheduleDate);
		}
		cursor.close();
		return null;
	...
	public ArrayList<ScheduleEntity> getAllSchedule(){
		...
		while(cursor.moveToNext()){
			...
			String scheduleContent = cursor.getString(cursor.getColumnIndex("scheduleContent"));
			String scheduleDate = cursor.getString(cursor.getColumnIndex("scheduleDate"));
			ScheduleEntity vo = new ScheduleEntity(scheduleID,scheduleTypeID,remindID,scheduleContent,scheduleDate);
			list.add(vo);
		}
		cursor.close();
	 ...
	 * @param vo
	 */
	public void update(ScheduleEntity vo){
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		ContentValues values = new ContentValues();
